...
import cordova.plugin.qnrtc.ui.LocalVideoView;
import cordova.plugin.qnrtc.ui.RTCVideoView;
...
public class RoomActivity extends Activity implements QNRoomEventListener, ControlFragment.OnCallEvents {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(QNRtc.getResourceId("activity_room", "layout"));//R.layout.activity_room
        ...
        mLocalWindow = (LocalVideoView) findViewById(QNRtc.getResourceId("local_video_view", "id"));//R.id.local_video_view
        mLocalWindow.setUserId(mUserId);

        mRemoteWindowA = (RTCVideoView) findViewById(QNRtc.getResourceId("remote_video_view_a", "id"));//R.id.remote_video_view_a
        mRemoteWindowB = (RTCVideoView) findViewById(QNRtc.getResourceId("remote_video_view_b", "id"));//R.id.remote_video_view_b
        mRemoteWindowC = (RTCVideoView) findViewById(QNRtc.getResourceId("remote_video_view_c", "id"));//R.id.remote_video_view_c
        mRemoteWindowD = (RTCVideoView) findViewById(QNRtc.getResourceId("remote_video_view_d", "id"));//R.id.remote_video_view_d
        mRemoteWindowE = (RTCVideoView) findViewById(QNRtc.getResourceId("remote_video_view_e", "id"));//R.id.remote_video_view_e
        mRemoteWindowF = (RTCVideoView) findViewById(QNRtc.getResourceId("remote_video_view_f", "id"));//R.id.remote_video_view_f
        mRemoteWindowG = (RTCVideoView) findViewById(QNRtc.getResourceId("remote_video_view_g", "id"));//R.id.remote_video_view_g
        mRemoteWindowH = (RTCVideoView) findViewById(QNRtc.getResourceId("remote_video_view_h", "id"));//R.id.remote_video_view_h
        ...
        String[] hwBlackList = new String[]{"SM-G9209"};//getResources().getStringArray(QNRtc.getResourceId("hw_black_list", "array"));//R.array.hw_black_list
        ...
        ft.add(QNRtc.getResourceId("control_fragment_container", "id"), mControlFragment);//R.id.control_fragment_container
    ...
    private void startCall() {
        ...
        logAndToast(getString(QNRtc.getResourceId("connecting_to", "string"), mRoomId));//R.string.connecting_to
    ...
    private void onConnectedInternal() {
        ...
        logAndToast(getString(QNRtc.getResourceId("connected_to_room", "string")));//R.string.connected_to_room
    }
    ...
    private void disconnectWithErrorMessage(final String errorMessage) {
        new AlertDialog.Builder(this)
                .setTitle(getText(QNRtc.getResourceId("channel_error_title", "string")))//R.string.channel_error_title
                .setMessage(errorMessage)
                .setCancelable(false)
                .setNeutralButton(QNRtc.getResourceId("ok", "string"),//R.string.ok
    ...
    private void showKickoutDialog(final String userId) {
        if (mKickoutDialog == null) {
            mKickoutDialog = new AlertDialog.Builder(this)
                    .setNegativeButton(QNRtc.getResourceId("negative_dialog_tips", "string"), null)//R.string.negative_dialog_tips
                    .create();
        }
        mKickoutDialog.setMessage(getString(QNRtc.getResourceId("kickout_tips", "string"), userId));//R.string.kickout_tips
        mKickoutDialog.setButton(DialogInterface.BUTTON_POSITIVE, getResources().getString(QNRtc.getResourceId("positive_dialog_tips", "string")),//R.string.positive_dialog_tips
                new DialogInterface.OnClickListener() {
                    @Override
    ...
    public void onStateChanged(QNRoomState state) {
        ...
        switch (state) {
            case RECONNECTING:
                mCallStartedTimeMs = System.currentTimeMillis();
                logAndToast(getString(QNRtc.getResourceId("reconnecting_to_room", "string")));//R.string.reconnecting_to_room
                break;
            case CONNECTED:
                break;
        }
        ...
        switch (errorCode) {
            case ERROR_KICKED_OUT_OF_ROOM:
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        ToastUtils.s(RoomActivity.this, getString(QNRtc.getResourceId("kicked_by_admin", "string")));//R.string.kicked_by_admin
                    }
                });
                onCallHangUp();
                break;
    ...
    public void onStatisticsUpdated(QNStatisticsReport report) {
        ...
        //R.string.log_text
        mLocalLogText = String.format(getString(QNRtc.getResourceId("log_text", "string")), report.userId, report.frameRate, report.videoBitrate / 1000, report.audioBitrate / 1000, report.videoPacketLostRate, report.audioPacketLostRate, report.width, report.height);
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
